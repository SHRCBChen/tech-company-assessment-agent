...
将企业/项目名单和大赛现场自由笔记交给WorkBuddy后，WorkBuddy按照统一检索口径逐家开展公开信息深检，并从同一份事实记录生成四类成果。名单和现场笔记既可以放在同一个Excel，也可以作为多个文件分别上传：

1. 11类企业信息Excel；
2. 一家企业一条可编辑笔记形式的ima当前事实底稿；
3. 1—2页企业初评报告；
4. 按固定模板生成的推进意见。

后续收到分支行拜访记录或企业材料时，可在原批次上更新事实记录，再同步重建四类成果。
...
正式验收还要求：渠道审计绑定具体查询或页面和来源ID，官网优先由企查查详情链接进入并检查具体内容页；客户关系登记主体与阶段；投资机构背景使用独立于融资事件的来源；每家企业完成历史融资扫描。
...
| 推进意见 | `deliverables\advancement` |
| 逐家事实记录 | `records` |

`records`是四类成果的统一生成源，不要只修改交付Excel。否则下次重建时，Excel中的手工修改会丢失。
...
5. 先读取一家企业的`deliverables\ima-ready\*｜当前事实底稿.md`正文，创建Markdown格式的ima可编辑笔记，再把该笔记关联到知识库；不要把`.md`仅作为普通文件上传；
6. 在ima中打开笔记确认可编辑，再用企业全称和一条关键事实检索，确认标题、中文内容和索引正常；
7. 验证通过后，再让WorkBuddy把本批次其余`ima-ready`底稿按同样方式创建为笔记并关联到同一知识库。

推荐指令：

```text
请使用已连接的ima连接器，读取当前批次deliverables/ima-ready目录中的Markdown正文，逐家创建可编辑ima笔记，并以笔记类型关联到“大赛企业知识库”，不要把Markdown仅作为普通文件上传。先处理一家，确认笔记可编辑并通过企业全称和关键事实检索验证；成功后再处理其余企业。遇到同名笔记、覆盖、替换、删除或权限问题时立即告诉我具体企业，不要自行删除旧笔记。
```

### （二）备用方式：本地OpenAPI脚本

仅当所在WorkBuddy版本没有可用ima连接器，并且单位允许使用OpenAPI时，才运行`sync-approved-run-to-ima.ps1`。该脚本使用`import_doc → add_knowledge(media_type=11)`创建可编辑笔记，不再通过COS上传Markdown文件。API Key和Client ID不得写入Excel、聊天或GitHub。换电脑后需重新设置本机凭据。
