...
同名文件不得重复上传，记录`ima_replace_required`。处理后用企业全称和关键事实检索，验证文件名、中文编码和索引内容。

## 文件结构

ima采用“一家企业一个当前事实底稿文件”，不为每家企业建立文件夹。底稿保存全部证据事实、来源边界、现场笔记和评级简因；Excel和报告是给客户经理的交付成果，不把完整评价或拜访重点放入知识库。
