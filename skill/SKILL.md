...
description: 批量导入科创企业或项目名单及大赛现场自由笔记（名单可为Excel/CSV，笔记支持同一Excel、独立Excel/CSV、Word .docx、图片或PDF），逐家完成主体映射、六轮公开深检和跨列事实提取，并由同一事实记录生成11类企业信息Excel、ima当前事实底稿及1—2页企业初评报告；也支持导入分支行拜访记录或企业材料后同步更新三类成果。适用于“导入企业名单后一键检索”“名单和现场笔记分开上传”“交付三件成果”“继续上次批次”“拜访后更新Excel/知识库/报告”等任务。
---
...
- 新名单：支持企业/项目名称与现场笔记在同一Excel同一行，也支持名单Excel/CSV与多个笔记附件分开上传。Excel/CSV笔记按名称列绑定；Word(.docx)按企业名称或唯一简称标题分段；图片/PDF必须先视觉识别文字，再按其中的企业/项目名称绑定。未匹配笔记、重名冲突、Word未绑定段落、OCR不清和缺失笔记必须显式列出，不得仅凭文件名或上下文猜测归属。现场笔记是首批高价值材料，不是后续增量或“企业交流记录”。
...
2. 对每家企业：
   - 原样保存现场笔记（图片/PDF同时保留原件路径与OCR原文），再拆为最小事实单元；把人名、型号、客户、投资方、轮次、资质、专利、量产及经营指标加入检索锚点。
