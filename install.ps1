...
Copy-Item -LiteralPath (Join-Path $repoRoot "skills\gpt-codex\SKILL.md") -Destination (Join-Path $SkillInstallPath "SKILL.md") -Force
New-Item -ItemType Directory -Force -Path (Join-Path $SkillInstallPath "agents"),(Join-Path $SkillInstallPath "references") | Out-Null
Copy-Item -LiteralPath (Join-Path $repoRoot "skills\gpt-codex\agents\openai.yaml") -Destination (Join-Path $SkillInstallPath "agents\openai.yaml") -Force
Get-ChildItem -LiteralPath (Join-Path $repoRoot "skills\gpt-codex\references") -File | Copy-Item -Destination (Join-Path $SkillInstallPath "references") -Force
