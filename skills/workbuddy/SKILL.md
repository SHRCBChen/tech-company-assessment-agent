...
5. 初评报告完成后回查统一事实记录，再按[成果、评级与报告](references/outputs-rating-and-report.md)专项检索市场规模、行业趋势和行业内融资案例，核验后回写事实记录，再使用Skill内置的[推进意见模板](assets/推进意见模板.docx)生成DOCX；用户另行提供新版模板时以用户文件为准。必须从模板副本定点替换内容，保留52段正文、3张表、原章节标题及原槽位的字体、字号、加粗、缩进、行距和表格格式，不得另套全局样式。删除模板中的示例产业链图片后，只在原有三个段落槽位分别写上游、企业所处环节和下游/产业化情况。不得为了版面省略已确认股东、投资方或关键事实。
