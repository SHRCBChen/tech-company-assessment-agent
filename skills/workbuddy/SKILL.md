...
description: 在WorkBuddy中批量导入科创企业或项目名单及大赛现场自由笔记，优先调用官方企查查连接器并结合公开网页完成主体映射、六轮深检和跨列事实提取，再生成企业信息Excel、ima可编辑当前事实底稿、1—2页企业初评报告和模板化推进意见；也支持拜访后同步更新四类成果。适用于WorkBuddy中的“一键检索企业名单”“调用企查查连接器”“交付四件成果”“继续批次”和“拜访后更新”等任务。
---
...
5. 统一事实记录是四类成果的事实中枢，但不是推进意见专项研究的终点。Excel、ima底稿和初评报告完成后，推进意见后置开展市场规模、行业趋势和行业内股权融资案例的补充公开检索；核验后的新增事实先回写统一事实记录，再进入推进意见。不得从Excel或报告机械转写，也不得绕开事实中枢直接把检索片段写入成稿。旧成果只能作为检索锚点或回归测试，不能反向覆盖当前事实记录。
...
3. 只有六轮、11类事实字段审计、公开风险事项核验、现场笔记拆解与交叉核验、三类专项深化、跨列扫描和三队列全部完成，企业才能标`researched`；主体未闭合则标`mapping_blocked`。
   - “公开风险事项”优先用官方企查查连接器核验目标主体自身风险，排除关联风险和历史提示；底稿保留查询时点、类型和来源，Excel仅填`有`或`无`。
...
5. 初评报告完成后回查统一事实记录，再按[成果、评级与报告](references/outputs-rating-and-report.md)专项检索市场规模、行业趋势和行业内融资案例，核验后回写事实记录，再生成推进意见。无论使用Skill内置模板还是用户提供的新版模板，均须先执行：`python scripts/prepare_advancement_template.py --input "<原模板.docx>" --output "<批次目录>/work/推进意见无示例图模板.docx"`；只能从该“无示例图模板”复制企业文件。完成每一份DOCX后必须执行：`python scripts/prepare_advancement_template.py --check --input "<企业推进意见.docx>"`。检查失败时，丢弃该企业成品并重新从无示例图模板生成，不得靠手工逐个删除图片或图形后交付。保留52段正文、3张表、原章节标题及原槽位的字体、字号、加粗、缩进、行距和表格格式，不得另套全局样式。只在原有三个正文槽位分别写上游、企业所处环节和下游/产业化情况；除非用户明确提供企业专属图片并要求纳入，推进意见不得含任何图片、流程图或装饰图。不得为了版面省略已确认股东、投资方或关键事实。
6. 对报告和推进意见执行`Humanizer-zh`：删除空泛宣传、机械罗列、模板化连接词和无信息量套话；报告结构与推进意见模板均不得改变。
7. 本地质检通过后，把`ima-ready` Markdown正文创建为可编辑ima笔记，再以笔记类型关联到“大赛企业知识库”；不得把Markdown作为普通附件上传。遇到同名笔记、权限、登录或缺失企查查导出等需要用户动作时立即说明。
